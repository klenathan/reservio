...
function DropZone(props: IDropZone, {...rest}) {
    ...
    const {getRootProps, getInputProps} = useDropzone({
        ...
        onDrop: (acceptedFiles) => {
            if (props.multiple) {
                const newFiles = acceptedFiles.filter((file) => !files.find((prevFile) => prevFile.name === file.name));
                ...
                resizeImage(acceptedFiles[0], acceptedFiles[0].name, (resizedFile : any) => {
    ...
    function resizeImage(file: File, name: string, callback: (resizedFile: File) => void) {
    ...
    const thumbs = files.map((file) => {
        return (
            <div className={"flex flex-col items-center"} key={file.name}>
                <div className={"relative w-36 h-36 md:w-60 md:h-52"}>
                    <Image src={file.preview} alt={file.name} fill={true} className={"object-contain"}/>
                </div>
                <button
    ...
    return (
        <section className="container h-fit w-full mx-auto">
            ...
            <div className={"w-full grid grid-cols-2 gap-2 md:grid-cols-3 md:gap-3 justify-items-center"}>{thumbs}</div>
        </section>
    );
}
