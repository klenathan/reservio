...
export default function Category(slugs: any) {
    ...
    const handleSort = (childData: any) => {
        ...
        if (slugs.searchParams.category) {
            setUrl(
                `/service?category=${slugs.searchParams.category || ""}&minPrice=${
                    getValues("minPrice") || ""
                }&maxPrice=${getValues("maxPrice") || ""}&fromDate=${
                    getValues("fromDate") || ""
                }&toDate=${getValues("toDate") || ""}&${childData.sortBy}=${
                    childData.order
                }`
            );
        } else if (slugs.searchParams.keyword) {
            setUrl(
                `search?query=${slugs.searchParams.keyword}&${childData.sortBy}=${childData.order}`
            );
        }
    };
    ...
    const onSubmit: SubmitHandler<IFromInput> = async (data) => {
        ...
        setUrl(
            ...
            }&minPrice=${minPrice}&maxPrice=${maxPrice}&fromDate=${fromDate}&toDate=${toDate}&${
                sortBy.sortBy
            }=${sortBy.order}`
        );
    };
    ...
    return (
        <div className="overflow-hidden">
            ...
            <div className={"flex w-full justify-center pt-4 min-h-screen"}>
                <div className="flex w-full flex-col lg:flex-row 2k:w-[calc(100vw_-_20rem)] h-full ">
                    {aside && (
                        <aside
                            ref={isFirstOpenRef}
                            className="lg:self-auto h-fit self-center mb-2 lg:mb-0 p-4 mt-1 border border-black lg:w-1/5 mx-5 lg:ml-0 lg:flex"
                        >
                            <Form
                        ...
                        <div className="flex-1 min-w-0 overflow-auto">
                            ...
                            {queryService.length > 0 ? (
                                <div className="flex-1 w-full flex flex-col items-center space-y-5">
                                    ...
                                    <div className={"lg:mr-6 lg:ml-0 mx-6 space-y-4"}>
                                        <Sort
                                            sortOptions={sortOptions}
                                            parentCallBack={handleSort}
