...
const TotalPrice = (props: TotalPriceProps) => {
  ...
  const transformValue = (value: number) => {
    if (value > availableQuantity) {
      ...
      setError("quantity", {
        type: "invalid",
        message: `${availableQuantity} slots only 😵`,
      });
    }
  };
  ...
  const makeReservation = async () => {
    ...
    props.productFixedTimeSlotId &&
      reservationData.append(
        "productFixedTimeSlotId",
        props.productFixedTimeSlotId
      );
    ...
    >
      ...
      >
        ...
        <div className={"grid grid-cols-2 outline outline-1 outline-black"}>
          ...
          {props.endDate ? (
            <ColInformation smallText={"End date"} mainText={endDateString} />
          ) : (
        ...
        {/*Discount*/}
        <RowInformation smallText={"Discount"}>
          <div
            className={
              "flex font-bold justify-center w-full bg-neutral-100 hover:bg-neutral-200 cursor-pointer"
            }
          ...
          <Modal
            nameModal={"🈹 Choose your discount 🈹"}
            isOpen={isModalOpen}
          ...
          >
            ...
            <div
              className={
                "overflow-auto h-full max-h-96 space-y-4 py-6 px-1.5 md:px-6 lg:px-10 snap-both snap-mandatory scroll-smooth"
              }
            >
              {data &&
                data.map((discount) => (
      ...
      {props.isLogin ? (
        ...
        <div className={"flex mt-2 p-3 justify-center"}>
          <Button btnStyle={"filled"} onClick={() => push("/login")}>
            Login for book me 😘
          </Button>
        </div>
      )}
