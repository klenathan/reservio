...
const DetailPage = (props: { service: IService }) => {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.NEXT_PUBLIC_BACKEND_ENDPOINT}user/${props.service.vendorUsername}`
      )
      .then((res) => {
        console.log(res);
        setVendorInfo({
      ...
      {vendorInfo ? (
        <div className={"flex flex-row  space-x-4 mt-7 h-36"}>
          <div className={"w-20 h-20 relative"}>
            <Picture
              src={process.env.NEXT_PUBLIC_IMG_ENDPOINT + vendorInfo.avatar}
            />
          </div>
          <div className={"flex flex-col "}>
            <div className={"text-lg font-bold"}>
              Service by {props.service.vendorUsername}
            </div>
