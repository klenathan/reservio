        ...
        <div className={'flex flex-col space-y-2'}>
            ...
            <div className={'flex flex-col md:flex-row justify-around'}>
                <Datepicker
                    ...
                    // showFooter={true}
                    containerClassName={"relative w-full md:w-96 border border-limeGreen rounded-lg max-h"}
                    inputClassName={
...
const AddDateTime = (props: addDateTimeProps) => {
    ...
    useEffect(() => {
        ...
        if (dateStart && startTime) {
            date = getUnixTime(set(new Date(dateStart.startDate), {hours: startTime.hour, minutes: startTime.minutes}))
        }
    ...
    useEffect(() => {
        let date;
        if (dateEnd && endTime) {
            date = getUnixTime(set(new Date(dateEnd.startDate), {hours: endTime.hour, minutes: endTime.minutes}))
        }
        props.endDate(date, props.index)
    }, [dateEnd, endTime])
    return (
        <div className="flex flex-col justify-between space-y-3 border-2 border-neutral-300 shadow-md rounded-lg py-4 px-6">
            {/*Start Date*/}
            <AddingDate handleDateChange={handleDateStart} name={"Start"} dateRange={dateStart}
                        handleTimeCallBack={handleTimeCallBack}/>
            {/*End Date*/}
            <AddingDate handleDateChange={handleDateEnd} name={"End"} dateRange={dateEnd}
                        handleTimeCallBack={handleTimeCallBack}/>
            {/*Quantity*/}
            <Input
                className="mr-1"
